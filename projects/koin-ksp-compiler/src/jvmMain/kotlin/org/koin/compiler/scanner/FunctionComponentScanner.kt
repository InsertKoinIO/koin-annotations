...
class FunctionComponentScanner(
    ...
    val onClassDefinitionFound: (KSClassDeclaration) -> Unit
) : FunctionScanner(isModuleFunction = false) {

    fun createFunctionDefinition(
        element: KSAnnotated
    ): KoinMetaData.Definition? {
        ...
        return returnedType?.let {
            ...
            if (ksFunctionDeclaration.parent is KSClassDeclaration && functionName == "<init>"){
                // case of @Inject constructor()
