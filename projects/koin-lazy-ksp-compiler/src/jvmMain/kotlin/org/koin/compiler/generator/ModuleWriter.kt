...
abstract class ModuleWriter(
    ...
    fun writeModule(isComposeViewModelActive: Boolean) {
        ...
        if (generateModuleBody) {
            ...
            if (module.definitions.any { (it as? KoinMetaData.Definition.ClassDefinition)?.isLazy == true }) {
                writeLazyModuleFunction()
                writeLazyModuleIncludes()
                if (!hasExternalDefinitions) {
                    writeDefinitions(true)
                }
                writeLazyModuleFooter()
                onFinishWriteModule()
    ...
    open fun writeLazyModuleFunction() {
        // Module definition
        // TODO generate Extensions
        writeln(generateLazyModuleField(module))
    }
    ...
    private fun generateLazyModuleField(
        module: KoinMetaData.Module
    ): String {
        with(module) {
            val visibilityString = visibility.toSourceString()
            return "${visibilityString}val ${generatedField}Lazy : LazyModule get() = lazyModule {"
        }
    }
    ...
    open fun writeLazyModuleIncludes() {
        if (module.lazyIncludes?.isNotEmpty() == true) {
            generateIncludes()?.let { writeln("${TAB}includes($it)") }
        }
    }
    ...
    open fun writeDefinitions(considerLazy : Boolean = false) {
        val (standardDefinitions, scopeDefinitions) = module.definitions.filter {
            if(considerLazy) {
                (it as? KoinMetaData.Definition.ClassDefinition)?.isLazy == true
            } else {
               (it as? KoinMetaData.Definition.ClassDefinition)?.isLazy == false
            }
        }.partition { it.isNotScoped() }
    ...
    open fun writeLazyModuleFooter() {
        writeln(MODULE_FOOTER)
        val visibilityString = module.visibility.toSourceString()
        writeln("${visibilityString}val ${modulePath}.lazyModule : org.koin.core.module.LazyModule get() = ${generatedField}Lazy")
    }
